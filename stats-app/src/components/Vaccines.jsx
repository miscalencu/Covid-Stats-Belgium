...
const Vaccines = () => {
    ...
    const [ filterDose, setFilterDose ] = useState("ALL");
    ...
    const loadData = (pageSize, pageNr, orderBy, orderDir, loadChartData) => {
        ...
        _data.get(
        {
            url: `${process.env.REACT_APP_API_ROOT_URL}/Stats/GetVaccinesDate`,
            pageNr: pageNr,
            pageSize: pageSize,
            orderBy: orderBy,
            orderDir: orderDir,
    ...
    useEffect(() => {
        // set filter items
        _data.get({ url: `${process.env.REACT_APP_API_ROOT_URL}/Stats/GetFilterData?type=VaccinesDate&field=region` }, (data) => setRegions(data));
        _data.get({ url: `${process.env.REACT_APP_API_ROOT_URL}/Stats/GetFilterData?type=VaccinesDate&field=ageGroup` }, (data) => setAgeGroups(data));
        _data.get({ url: `${process.env.REACT_APP_API_ROOT_URL}/Stats/GetFilterData?type=VaccinesDate&field=sex` }, (data) => setSexes(data));
        _data.get({ url: `${process.env.REACT_APP_API_ROOT_URL}/Stats/GetFilterData?type=VaccinesDate&field=dose` }, (data) => setDoses(data));

        loadData(gridProps.pageSize, gridProps.pageNr, gridProps.orderBy, gridProps.orderDir, true);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [ filterStartDate, filterEndDate, filterRegion, filterAgeGroup, filterSex, filterDose ]);
        ...
        <>
                ...
                <Form.Row>
                    ...
                    <Col md={2}>
                        <Form.Control 
                            ...
                            <Form.Control as="option" value="ALL">Dose</Form.Control>
                            {
                                doses.map(item => {
                                    return <Form.Control as="option" key={item} value={item}>{item}</Form.Control>
                                })
            ...
            </Form>
            {
                ...
                <Grid 
                    ...
                    <Column sortable header='Dose' className='center' field='dose' />
                    <Column sortable header='Vaccines' className='center' field='count' />
                </Grid>
            }
            <StatsChart
                type="VaccinesDate"
